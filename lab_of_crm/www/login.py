...
def get_context(context):
	...
	allowed_messages = ["Login failed. Please try again."]
	msg = frappe.local.request.args.get("message")

	context.message = msg if msg in allowed_messages else ""
...
def custom_login():
    ...
    try:
        ...
        if not user or not pwd:
            raise frappe.AuthenticationError("Missing credentials")
        ...
        frappe.response["location"] = frappe.form_dict.get("redirect-to") or "/app"
